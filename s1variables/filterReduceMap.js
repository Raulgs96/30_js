let products = [
  ...
  //      1.- Necesito una nueva lista, que contenga los productos que estan bajos en stock (<25) (filter)
const lowInStock = (arrayList) =>{
    let newList = []
    arrayList.filter((producto) => producto.stock < 25 ? newList.push(producto) : "")
    return newList
}

let prueba = lowInStock(products)
console.log(prueba)

//2.- Necesito una nueva lista que contenga los productos, únicamente con su nombre, y el precio real después de aplicar el descuento (map)
const discountPrice = (array) =>
    array.map(({name,price,discount})=>{
        let realPrice = price - price*discount
        let newList = {name,realPrice}
        return newList
    })


let prueba2 = discountPrice(products)
console.log(prueba2)


//3.- Necesito saber el precio promedio de los productos (reduce)
const averageProducts = (dataArray) =>
    dataArray.reduce((accum,producto)=> accum+producto.price,0)/ dataArray.length.toFixed(2);


let prueba3 =averageProducts(products)
console.log(prueba3)
